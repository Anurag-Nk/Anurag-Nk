# Hi, I'm Anurag 👋

### Data Analyst | SQL • Python • Tableau • Data Visualization

I'm an aspiring **Data Analyst** passionate about turning raw data into meaningful insights and building interactive dashboards that help people understand data better.

I enjoy working with data, solving problems using **SQL**, creating visual stories with **Tableau**, and continuously improving my analytical and technical skills.

---

## 🛠️ Skills & Tools

**Languages & Querying**

* SQL
* Python
* C

**Data Analytics & Visualization**

* Power BI
* Tableau
* Excel
* Data Cleaning
* Exploratory Data Analysis
* Data Visualization
* Dashboard Development

**Tools**

* Git & GitHub
* VS Code
* PostgreSQL

---

## 📊 What I'm Currently Working On

* 🧠 Strengthening my **SQL problem-solving and query-building skills**
* 📈 Building interactive **Tableau dashboards**
* 🐍 Improving my **Python for data analysis**
* 💼 Building practical data analytics projects for my portfolio
* 🚀 Preparing for opportunities in **Data Analytics & IT**

---

### 🗄️ SQL Practice & Analysis

A collection of SQL exercises covering filtering, sorting, aggregation, joins, subqueries, and analytical problem-solving.

**Tools:** SQL • MySQL

---

## 📚 Currently Learning

```text
SQL                ███████████████████░  Advanced
Tableau            ██████████████████░░  Advanced
Python             ████████████░░░░░░░░  Intermediate
Excel              ███████████████░░░░░  Intermediate
Data Analytics     ███████████████░░░░░  Intermediate
```

---

## 🎯 My Goal

> **Turn data into insights, insights into decisions, and continuously grow as a data professional.**

I'm currently looking for opportunities where I can apply my analytical skills, work with real-world data, and continue learning from experienced teams.

---

## 📫 Connect With Me

* 💼 [LinkedIn](https://www.linkedin.com/in/anuragnagarkoti/)
* 📧 Email: nkanurag2005@gmail.com

---

⭐ **Feel free to explore my repositories and follow my journey in data analytics!**
